"""
RMSD vs fraction of coil residues (scatter + regression, PDF output).

Replicates the consistent manuscript style used across plots.

CSV columns used: percent_coil (0–100, converted to fraction), rmsd
"""
...
OUT_PDF_PATH = \
    "/mnt/hdd8/mehdi/projects/vq_encoder_decoder/plots/rmsd_vs_ratio_coil_cluster50.pdf"
...
def main() -> None:
    ...
    if "percent_coil" not in df.columns or "rmsd" not in df.columns:
        raise KeyError("CSV must contain 'percent_coil' and 'rmsd' columns")

    x = (_coerce_numeric(df["percent_coil"]) / 100.0).to_numpy(dtype=float)
    ...
    ax.set_xlabel("Fraction of coil residues", fontsize=14)
